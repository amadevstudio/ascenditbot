...
class AllowedUser(Service):
    ...
    async def find(allowed_user_id: int) -> AllowedUserInterface | None:
        return await allowed_user_repository.find(allowed_user_id)

    @staticmethod
    async def switch_active(allowed_user_id: int) -> bool | None:
        return await allowed_user_repository.switch_active(allowed_user_id)

    @staticmethod
    async def delete(allowed_user_id: int) -> int | None:
        return await allowed_user_repository.delete(allowed_user_id)

    @staticmethod
    # Future improvements: cache in memory (redis)
    async def can_write_to_chat(nickname: str, chat_service_id: str) -> bool:
        # Return 'can write' if chat not active
        chat_monitoring_active: bool = await chat_repository.is_active_by_service_id(str(chat_service_id))
        if not chat_monitoring_active:
            return True

        # Check if connection exists
        privileges: AllowedUserInterface | None = \
            await allowed_user_repository.get_privileges(nickname, chat_service_id)
